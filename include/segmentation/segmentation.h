...
#ifndef PCD_COMPARE_SEGMENTATION_H
#define PCD_COMPARE_SEGMENTATION_H
using namespace std;
struct PointAPR{
	float azimuth;
	float polar_angle;
...
public:
	...
	static bool compare_cluster(pair<int,int> a,pair<int,int> b){
		return a.second>b.second;
	}//升序
	
	//cloud_IN == vapr size & id
	template <typename PointT>
	void calculateAPR(const pcl::PointCloud<PointT>& cloud_IN, vector<PointAPR>& vapr){
		
		//读取参数
		YAML::Node config = YAML::LoadFile("/media/echo/DataDisc/3_program/mapping/cfg/objectSeg.yaml");
		minrange = config["minrange"].as<float>();
		maxrange = config["maxrange"].as<float>();
		minazimuth = config["minazimuth"].as<float>();
		maxazimuth = config["maxazimuth"].as<float>();
		deltaA = config["deltaA"].as<float>();
		deltaR = config["deltaR"].as<float>();
		deltaP = config["deltaP"].as<float>();
		length = config["length"].as<int>();
		...
		cout<<"calculateAPR vapr.size(): "<<vapr.size()<<" cloud_IN .size(): "<<cloud_IN.size()<<endl;
	...
	void build_hash_table(const vector<PointAPR>& vapr, unordered_map<int, Voxel> &map_out){
		...
		cout<<"azimuth_index: "<<*maxPosition<<" range_index: "<<*maxPosition1<<" polar_index: "<<*maxPosition2<<endl;
	...
	bool most_frequent_value(vector<int> values, vector<int> &cluster_index) {
		...
		cout<<"cluster_index: "<<cluster_index.size()<<endl;
		return true;
	}
...
#endif //PCD_COMPARE_SEGMENTATION_H
