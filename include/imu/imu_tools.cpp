...
int imu_tools::voidIMUupdate(float gx, float gy, float gz, float ax, float ay, float az) {
...
void imu_tools::imu_intergration_euler(Eigen::VectorXd imu_mesurement, Eigen::Vector3d &vel_in,Eigen::Quaterniond &q_in,double period_time) {
	Eigen::Quaterniond Qwb;
	Qwb = q_in;
	Eigen::Vector3d gyro,acc,Pwb,Vw;
	Vw = vel_in;
	
	gyro[0] = imu_mesurement[3];
	gyro[1] = imu_mesurement[4];
	gyro[2] = imu_mesurement[5];
	acc[0]  = imu_mesurement[0];
	acc[1]  = imu_mesurement[1];
	acc[2]  = imu_mesurement[2];
	gyro = gyro - bias_gyro;
	float dt = period_time;
